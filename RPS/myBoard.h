...
class MyBoard : public Board
{
	const UINT rows;
	const UINT cols;
	BoardPiece*** table;	//2D array of pointers to pieces
...
public:
	...
	@returns -
			a pointer to a BoardPiece, if there is one in (col, row)
			a null pointer, if cell is empty, or illegal (col, row) input (exceeds the size of the board)
	*/
	BoardPiece* getPieceAt(UINT col, UINT row);

	/*
	This function sets a given pointer to BoardPiece at the postion (col, row) on board, does nothing if illegal arguments.
	*/
	void setPieceAt(BoardPiece* p, UINT col, UINT row);

	/*
	If input (col, row) is  legal (within boundries of board), delete the BoardPiece in position and reinitiaize position
	to nullptr
	*/
	...
	This function will find the BoardPiece placed at (fromX, fromY) on board, and if the piece is there, and owned
	...
	*/
	...
	/*
	This function recieves a pointer to BoardPiece and a position to place it on board.
	...
	If the position is already taken by a different BoardPiece of a different Player, than a match will take place.
		@returns -
			0 - function finished with no errors.
			1 - function finished with error, or a player has lost due to an illegal action
	*/
	int positionPiece(BoardPiece* p, UINT toX, UINT toY, unique_ptr<MyFightInfo>& fight, int moved = 0, UINT fromX = 0, UINT fromY = 0);
