...
class RSPPlayer_3 : public PlayerAlgorithm
{
private:
...
public:
	RSPPlayer_3(UINT boardRows, UINT boardCols,
		UINT R, UINT P, UINT S, UINT B, UINT J, UINT F, int ID);

	~RSPPlayer_3();

	RSPPlayer_3() : RSPPlayer_3(BOARD_SIZE, BOARD_SIZE, R_COUNT, P_COUNT, S_COUNT, B_COUNT, J_COUNT, F_COUNT, 0)
	{
...
int RSPPlayer_3::fillCornersWithAlreadyOccupiedCorners(vector<bool>& selectedCorners, BoardSet& boardSet)
{
	int count = 4;
...
void RSPPlayer_3::placeMovingPiecesOnCorners(vector<char> &movingPieceVector,
...
void RSPPlayer_3::chooseCorner(int select, RandomContext& rndCtx,
...
void RSPPlayer_3::fillListWithRPSBpieces(vector<char>& rpsbVector, const int bombsUsed)
{
	addAllToVector(R, 'R');
	addAllToVector(P, 'P');
	addAllToVector(S, 'S');
...
int RSPPlayer_3::positionFlagsAndBombs(RandomContext& rndCtx,
...
unique_ptr<Move> RSPPlayer_3::getNextRandomMove(void)
{
	random_device				seed;
	mt19937						gen(seed());
...
unique_ptr<Move> RSPPlayer_3::checkAllAdjecentOpponents(const MyPiecePosition & piece, std::bitset<4>& boolVec, const int x, const int y)
{
...
char RSPPlayer_3::getNewJokerRep(const char oldJokerRep)
{
	char result;
	switch (oldJokerRep)
	{
...
unique_ptr<JokerChange> RSPPlayer_3::checkAllAdjecentOpponents(const MyPiecePosition & joker, const int x, const int y)
{
...
bool RSPPlayer_3::checkForAdjecentOpponent(const MyPiecePosition other)
{
...
unique_ptr<Move> RSPPlayer_3::getMove()
{
...
char RSPPlayer_3::getRandomJokerChahge(int rand, char jokerRep)
{
	switch (jokerRep)
	{
	case 'R':
...
unique_ptr<MyMove> RSPPlayer_3::getLegalMove(const MyPoint& point)
{
...
bool RSPPlayer_3::existsOnBoardSet(const MyPoint& point)
{
	return playerPieces.count(MyPiecePosition(point.getX(), point.getY())) > 0;
}

void RSPPlayer_3::removeOutOfBoundsDirections(const MyPoint& point, std::bitset<4>& legalFleeDirections)
{
	//[left, right, up, down]
	if (point.getX() == 1)
...
unique_ptr<MyMove> RSPPlayer_3::getLegalMove(const MyPoint& point, bitset<4>& legalFleeDirections)
{
...
RSPPlayer_3::RSPPlayer_3(UINT rows, UINT cols, UINT R, UINT P, UINT S, UINT B, UINT J, UINT F, int ID) : ID(ID)
{
...
void RSPPlayer_3::getInitialPositions(int player, PieceVector& vectorToFill)
{
	//update player's ID
	ID = player;
...
void RSPPlayer_3::notifyOnInitialBoard(const Board & b, const vector<unique_ptr<FightInfo>>& fights)
{
	int owner;
...
void RSPPlayer_3::notifyOnOpponentMove(const Move & move)
{
...
void RSPPlayer_3::notifyFightResult(const FightInfo & fightInfo)
{
	const MyPoint fightPoint = fightInfo.getPosition();
	const MyPiecePosition fightPos(fightPoint.getX(), fightPoint.getY());
	int fightWinner = fightInfo.getWinner();
...
unique_ptr<JokerChange> RSPPlayer_3::getJokerChange()
{
